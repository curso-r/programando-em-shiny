...
```{r, eval = FALSE}
# NÃO É POSSÍVEL RODAR ESTE EXEMPLO
...
Salvamos o valor devolvido pela função `mod_filtro_server` em um objeto chamado `dados`. Com esse valor é uma expressão reativa, utilizamos a notação `dados()` na hora de acessar o seu valor.

É importante ressaltar que um módulo só pode acessar os valores que estão no server de um app se você explicitamente enviar este valor como parâmetro (como fizemos no primeiro exemplo desta seção). O inverso também vale: o server do app só consegue acessar um valor criado dentro do server de um módulo se você retorná-lo explicitamente (como fizemos no exemplo anterior).

Por fim, também é possível passar argumentos a para UI de um módulo, de maneira análoga ao que fizemos com o `server`.
